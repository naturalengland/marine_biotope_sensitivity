...
std_6_cols_attrb <- dplyr::tibble(attrb_name = c("fid", "pkey", "HAB_TYPE", "hab_1", "hab_2", "hab_3", "sbgr_reg_id", "uncertainty_sim"),
                           attrb_descr = c(""))

# descriptions for ...:

# fid 
std_6_cols_attrb$attrb_descr[std_6_cols_attrb$attrb_name == "fid"] <- "Feature ID (unique identifier) assocaited with GIS file."
# pkey 
std_6_cols_attrb$attrb_descr[std_6_cols_attrb$attrb_name == "pkey"] <- "ID (unique identifier) assocaited with R processing routine."
# HAB_TYPE
...
# sbgr_reg_id
std_6_cols_attrb$attrb_descr[std_6_cols_attrb$attrb_name == "sbgr_reg_id"] <- "An ID number representing the respective sub-biogeoregions in English inshore waters as defined by Keith Hiscock in a project in which likely biotopes occuring in each of the sub-biogeoregions were assigned thereto."
# uncertainty_sim
std_6_cols_attrb$attrb_descr[std_6_cols_attrb$attrb_name == "uncertainty_sim"] <- "A score ranging from 0 - 1 which represents a quantitative estimation of the certainty assocaited with the biotope which was assigned to mapped habitat (HAB_TYPE). Zero (0) represents low certainty and one (1) represents high certainty."


# Print message
cat("A new object named, std_6_cols_attrb, is now loaded into R environment. It contains two columns which match 2 of the names of act_press_attribution_results, which will be appended into a new table containing ALL the attribute descriptions.")