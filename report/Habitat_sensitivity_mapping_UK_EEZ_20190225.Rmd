...
#### Marine habitat map
The benthic habitat map used by NE (Marine_Evidence_Base_Internal.gdb, layer = BSH), was used as the basis to inform the spatial distribution of benthic habitats around within the UK's EEZ for this project. The habitat categories contained within NE's habitat map are based on the EUNIS classification developed in conjunction with the JNCC [@Davies2004; @Galparsoro2012]. The habitat map is continuously updated as new information on habitat becomes available following a well defined process between NE and JNCC. The extent of the habitat map is: -24.2816553949999729,47.9878704700000753 : 3.7191888160000417,64.0702531440000485, in datum World Geodetic System 1984 (WGS84). The habitat map include over a million (1,207,497) polygons, which have been attributed with a habitat type category following the EUNIS classification. The map below shows the variety of habitats, and the differences in the levels of EUNIS habitats around the UK. the map also shows that more than one type of habitat may be associated with a given polygon. In this study, only the habitat category (EUNIS code) appearing first in the list of possible habitats was used in this study. Attempting to create sensitivity maps for all possible combinations, would cause significant additional computational constraints. The first occurring habitat was chosen, as it was the most likely habitat or the most abundant within the polygon (Timothy Fegan, Lead Advisor, Marine GI, Evidence Services Natural England, personal comment). For example, if a polygon had EUNIS code A1.1/A1.2, only A1.1 would have been used
...
eunis_mismatch_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/EUNIS_mismatch_20190319.png"
...
The spatial match of scallop dredging activity was compared to the distribution of sensitive habitats. The habitat sensitivity map to abrasion (pressure code D6) from dredging (activity code Z5) in the inshore area was used. Offshore sensitivity may be added in due course once JNCC data is incorporated in the analysis. The aggregated dredging activity between 2013-2014 stored in file "Fishermap_JNCC_NE/MMO_UK_VMS_Fishing_Activity.gdb" was used as a realistic and recent example of the geographic distribution of dredging pressure in the English EEZ. The above dredging activity data set contains scallop dredging, tractor drawn dredging, and rake dredging data, but the latter two only apply to the shoreline-area. Areas known for other kinds of dredging activity, such as hydrolic dredging at The Wash were excluded after the identification of areas with relatively high fishing pressure in the Results. Areas with relatively high level of dredging activity were identified by setting a minimum threshold of including cells from the activity map where dredging activity exceeded 20 % of the highest activity value. This was done to restrict the area of investigation to distinct (and disjointed) areas where fishing pressure was greatest. If no cut-off value was applied, the areas become very large and connected along he inshore 12NM boundary.
...
seafish_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/Port_based_landings_no_text_2017.png"
...
dredge_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/Fishing_pressure_dredge.png"
...
aois_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/AOI_Fishing_pressure_dredge.png"
...
hs_drdg_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/Habitat_sensitivity_abrasion_dredging.png"
...
Sub-biogeoregion 2c in the Southern North Sea and 5 in the Irish Sea had the greatest proportional area of highly sensitive habitat each covering about 1.6 % of the total inshore area. The subbiogeoregions 1a and 1b in the northern North Sea had medium sensitivity, and each covered over 30 % of the total inshore area.

In sub-biogeoregion 2c had the largest single polygon of high habitat sensitivity covering an area almost 1.6 % of the inshore habitat. This polygon had a habitat classification of sublittoral mixed sediments (A5.4), which was assessed as fine-scale habitat category *Cerianthus lloydii* and other burrowing anemones in circalittoral muddy mixed sediment (A5.441) against dredging pressure  (Z10_5)from abrasion pressure (D6) and had a high level of sensitivity with low degree of confidence in the assessment (Quality of Evidence). the parent habitat type (A5.4) had a medium to  high degree of confidence associated with a MESH score  of 66 out of 100. Future analysis will look at contiguous areas (connected polygons) using habitat patch statistics.
...
Note this this provides an example of one kind of activity and pressure.
...
The habitat sensitivity assessments include the assessed habitat, i.e. the fine-scale habitat associated with the broad-scale habitats within a sub-biogeoregion. 
...
hab_assessed_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/inshore_fine_assessed_habs.png"
...
conf_map_img_path <- "F:/projects/marine_biotope_sensitivity/report/figrues/confidence_map.png"
include_graphics(conf_map_img_path, dpi = 150)
```

The Quality of Evidence outputs are accompanied by mapable MESH scores, which together provide the user with spatial index of confidence in the assessments.
