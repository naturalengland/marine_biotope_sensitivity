...
### R-code steps
All programming to generate habitat sensitivity layers was carried out in R. the files are stored on Natural England's GitHub [webpage](https://github.com/naturalengland/marine_biotope_sensitivity). Here only the high-level overview of how the code works is described. Details about what each section of code does is braodly described in the Annex, and specific details are described within each R-file. The inshore and offshore are two separate sets of code, and run indecently, after they were split using the GIS described above. The R scripts of r the inshore and offshore follow largely the same steps, described below.

In essennce the R-code read the senstivity assesmsents (and assocaited confidfence scores) and GIS data into the R statistical environment (an instance of the R programme), where it associated the sensitivty assessments to the respective benthic habitat maps (inshore and offshore). In the offshore the R-code evalutes the fine-scale habtias which may occur wihtin broad-scale habitats, and associates the individual mapped habitat polygons to the fine-scale habtitat assessments with the highest senstivity score. For the offshore, it uses the habtiat code provided as is, and does not attempt to find fine-scale habtiats wihtin broad-scale habitats, for the reasons provided earlier. The data are then saved and exported to geopackage for the inshore and offshore components, respectively. 

## Case Study
### Fishing activity data investigation
The spatial match of scallop dredging activity was compared to the distribution of sensitive habitats. The habitat sensitivity map to abrasion (pressure code D6) from dredging (activity code Z5) in the UK's EEZ was used.The aggregated dredging activity between 2013-2014 stored in file "Fishermap_JNCC_NE/MMO_UK_VMS_Fishing_Activity.gdb" was used as a realistic and recent exmaple of the geographic distribution of this pressure in the English EEZ. The above dredging activity data set contains scallop dredging, tractor drawn dredging, and rake dredging data, but the latter two only apply to the shoreline-area. It can therefore be assumed that the dredging layer largely represents scallop dredging. In future this layer may be replaced by another, but here it is meant to be proof of concept for internal use. *Requires further infomration on background of Fishermap_JNCC_NE\MMO_UK_VMS_Fishing_Activity.gdb data to spell out details around the layer; and describe the values on the table*. 

The Marine Conservation Zones (MCZs) were overlaid onto the fishing activity map to to provide perspective of the location of activities and existing protected areas. Other forms of protected areas, SACs, SPAs may be added in future if the project further investigates the three-way relationship of senstive habitats, activities at sea and the location of protected areas. 
...
The Wash was characterised by having the highest levels of dredging activity in England. The offshore areas between Worthing and Eastbourne is the largest contiguous area with relatively high levels (>10.8) of dredging activity, but it was predominantly focussed in the offshore area (Figure 6). Other areas with relatively high levels of dredging activity include the Solent between Portsmouth and the Isle of Wight. Dredgers operating near Brixham had highest acativity levels inshore, but also a large area of medium-high activity in the offshore. A large area between Falmouth and Plymouth had relatively high levels of dredging activity associated with the inshore area. Between Newlyn and the Isles of Scilly, there was an area with relatively high levels of dredging activity straddled accross the inshore and the offshore. The offshore area between Isle of Man and Blackpool in the Irish Sea was another area with relatively high levels of fishing activity. Cardigan Bay had high levels of dredging, but was not further considered as it is outside of English waters.
...
## Sensitivity mapping
In total about 390 benthic habitat sensitivity maps were produced (not all activities-pressure-habitat combinations had valid interactions) for 39 pressures from the ten fishing activities, for the inshore and for the offshore respectively. Including all activities the R script has the capability to instantly produce similar data sets for 18 operations (for the inshore and offshore), but were not al lstored as indepedent data sets, as 1) this requires a significant amount of data storage space (aproximately 72 GB), and while this seems trivial with data storage capabilities today, the fact that these maps can and should be updated as new data becomes incorporated into the database and map set was sufficient reason to produce the maps as and when needed by running the code for specific requirements. All outputs are stored as geopackages at this stage, for which the output maps were vector based, and mirror the spatial resolution and habitat detail of the benthic habitat map. 

## Descriptive statistics to include:
1. Area per habitat sensitivity, for the inshore and offshore.
2. Area per habitat sensitivity, in MCZs.
3. Area per habitat senstivity in areas with "relatively high" levels of fishing pressure.

## Case study results

The spatial distribution of benthic habitat sensitivity to abrasion from dredging is shown in Figure 6. The offshore is characterised by large contiguous areas of medium or low level sensitivity. The inshore benthic habtiat is cahracterised by a patchwork of habitat senstivities including several highly sensitive habitats.
...
10. the outputs from step nine are used in step ten, where it retains only the habitat-sensitivity assessment of the assessed habitats which had the highest sensitivity score, per GIS polygon ID value. This set of data matrices are stored in an R object "sbgr.BAP.max.sens". 
11. The confidence assessments were assocaited with the resulting maps using a join between the EUNIS habitat codes inthe habitat map and that from the table where the EUNIs assessments are stored.
