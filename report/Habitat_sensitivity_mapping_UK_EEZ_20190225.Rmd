...
### Integration of expert fine-scale benthic habitats within inshore sub-biogeoregions 
...
```{r subbiogeoregions, fig.cap= "Figure 4: The subbiogeoregions within which fine-scale habitats (EUNIS level 4 - 6) are known, and were assocaited with broad-scale habitats from which fine scale habitat on the habitat map.",  echo = FALSE, , out.extra='style="background-color: #FFFFFF; padding:20px; display: inline-block;"'}
...
knitr::include_graphics(GIS_process_path, dpi = 200)
...
The spatial match of scallop dredging activity was compared to the distribution of sensitive habitats. The habitat sensitivity map to abrasion (pressure code D6) from dredging (activity code Z5) in the UK's EEZ was used.The aggregated dredging activity between 2013-2014 stored in file "Fishermap_JNCC_NE/MMO_UK_VMS_Fishing_Activity.gdb" was used as a realistic and recent example of the geographic distribution of this pressure in the English EEZ. The above dredging activity data set contains scallop dredging, tractor drawn dredging, and rake dredging data, but the latter two only apply to the shoreline-area. It can therefore be assumed that the dredging layer largely represents scallop dredging. In future this layer may be replaced by another, but here it is meant to be proof of concept for internal use. *Requires further infomration on background of Fishermap_JNCC_NE\MMO_UK_VMS_Fishing_Activity.gdb data to spell out details around the layer; and describe the values on the table*. 
...
2. Marine Conservation Zone (MCZ) layer (file): "Marine_Conservation_Zones_NE_CCW/Marine_Conservation_Zones_England.shp"

A fishing activity layer showing dredging and the current Marine Conservation Zone (MCZ) layer shows the placement of MCZs in relation to the fishing activity. The map shows that there was very limited spatial overlap between the dredged areas with high levels of activity (>10.8) and MCZs (Figure 6). 
...
## Areas with high levels of activity
A substantial inshore area was associated with relatively high levels of dredging activity, in particular the Wash. 
The Wash was characterized by having the highest levels of dredging activity in England. The offshore areas between Worthing and Eastbourne is the largest contiguous area with relatively high levels (>10.8) of dredging activity, but it was predominantly focused in the offshore area (Figure 6). Other areas with relatively high levels of dredging activity include the Solent between Portsmouth and the Isle of Wight. Dredgers operating near Brixham had highest activity levels inshore, but also a large area of medium-high activity in the offshore. A large area between Falmouth and Plymouth had relatively high levels of dredging activity associated with the inshore area. Between Newlyn and the Isles of Scilly, there was an area with relatively high levels of dredging activity straddled across the inshore and the offshore. The offshore area between Isle of Man and Blackpool in the Irish Sea was another area with relatively high levels of fishing activity. Cardigan Bay had high levels of dredging, but was not further considered as it is outside of English waters.
...
## Sensitivity mapping
In total about 390 benthic habitat sensitivity maps were produced (not all activities-pressure-habitat combinations had valid interactions) for 39 pressures from the ten fishing activities, for the inshore and for the offshore respectively. Including all activities the R script has the capability to instantly produce similar data sets for 18 operations (for the inshore and offshore), but were notallstored as independent data sets, as 1) this requires a significant amount of data storage space (approximately 72 GB), and while this seems trivial with data storage capabilities today, the fact that these maps can and should be updated as new data becomes incorporated into the database and map set was sufficient reason to produce the maps as and when needed by running the code for specific requirements. All outputs are stored as geopackages at this stage, for which the output maps were vector based, and mirror the spatial resolution and habitat detail of the benthic habitat map. 
...
1. Area per habitat sensitivity, for the inshore and offshore. (Use field: shape_area)
2. Area per habitat sensitivity, in MCZs. (can use fields in fields above and the  to work out the area per marine protected area)
3. Area per habitat sensitivity in areas with "relatively high" levels of fishing pressure. (Select values above 10.82, and attribute them with "aoi" field. Then dissoolve based on that field, and intersect/clip habitat sensitivity to establish the area with high fishing sensitivity
...
The spatial distribution of benthic habitat sensitivity to abrasion from dredging is shown in Figure 6. The offshore is characterized by large contiguous areas of medium or low level sensitivity. The inshore benthic habitat is characterized by a patchwork of habitat sensitivities including several highly sensitive habitats.
<br>
The offshore area in the English Channel is dominated by low sensitivity habitat, while the inshore area in the English Channel has several areas of medium to high sensitivity, such as Skerries Bank, and reef areas off Plymouth. The Solent area includes habitat with non-sensitive to highly sensitive between Portsmouth and the Ilse of Wight.

<br>

```{r insh_drdg, fig.cap= "Figure X : The habitat sensitivity to abrasion from inshore dredging.",  echo = FALSE, , out.extra='style="background-color: #FFFFFF; padding:20px; display: inline-block;"'}
...
The Quality of Evidence was provided as a spatial measure of confidence of the sensitivity assessments for the inshore and offshore.
