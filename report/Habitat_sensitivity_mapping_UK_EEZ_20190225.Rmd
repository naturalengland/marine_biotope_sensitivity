...
### Data processing
GIS steps were used to split the original English waters habitat map into an inshore (0 - 12 NM) and offshore component (12 - 200 NM). All other technical data manipulation was carried out using the R statistical environment in effort to make the data processing was transparrent, repeatable and easy to update outputs following changes to the sensitivity assessment database or updates to the benthic habitat map in future.

### GIS steps 
The inshore (0 - 12 NM) and offshore (12 - 200 NM) habitat areas were produced using geoprocessing tools in QGIS. A mask was created which delineates the inshore and areas, respectively (Figure Mask). The habitat map (Marine_Evidence_Base_Internal.gdb) was (step 1) separated into the inshore and offshore using a difference- intersection of the habitat map and a masks of the inshore and offshore (Figure Mask). This newly created inshore habitat layer was intersected with the sub-biogeoregional shapefile (step 2) associate the sub-biogeoregions (subbiogeoregions_clipped12nm_joined_sbgr_PH_wgs84.shp) to the individual polygons comprising the inshore habitat map (Figure 5).
<br>
```{r masks, fig.cap = "Figure Mask. The mask files used to preapre the habitat map to the English inshore and offshore territorial waters.", echo = False}
rpt_d <- getwd()
