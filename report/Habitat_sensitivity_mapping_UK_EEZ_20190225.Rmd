...
knitr::include_graphics(english_waters_img_path, dpi  = 80)
```
...
```
#### Sensitvity assessments database
...
Sensitivity confidence assessments provide an indication of the level of confidence associated with each sensitivity assessment using the MarESA methods. The confidence assessments provides three types of confidence assessments, Quality of evidence (QoE), Applicability of Evidence (AoE) and Degree of concordance between evidence (DoE) for each of the sensitivity components, namely, Resistance, Resilience and Sensitivity. The QoE provides a measure of the level of regard with which the evidence is considered, e.g. Peer review, grey literature, expert opinion. The AoE provides a measure if how realistic is it for a given pressure to impact upon a habitat. The DoE provides a measure of the agreement between different evidence sources. the higher the score, the greater the level of confidence

Only the quality of evidence (QoE) for overall sensitivity was displayed as a map in this study, but all measures are available for mapping. No attempt was made to amalgamate confidence indeces into single measures, as what they represent becomes lost when doing so, especially if this were to be combined with the spatial confidence of the habitat map.

#### Marine habitat map
The benthic habitat map used by NE (Marine_Evidence_Base_Internal.gdb, layer = BSH), was used as the basis to inform the spatial distribution of benthic habitats around within the UK's EEZ for this project. The habitat categories contained within NE's habitat map are based on the EUNIS classification developed in conjunction with the JNCC [@Davies2004; @Galparsoro2012]. The habitat map is continuously updated as new information on habitat becomes available following a well defined process between NE and JNCC. The extent of the habitat map is: -24.2816553949999729,47.9878704700000753 : 3.7191888160000417,64.0702531440000485, in datum World Geodetic System 1984 (WGS84). The habitat map include over a million (1,207,497) polygons, which have been attributed with a habiat ype category following the EUNIs classification. The map below shows the variety of habitats, and the differences in the levels of EUNIS habitats around the UK. the map also shows that mrope than one type of habitat may be assocaited with a given polygon. In this study, only the habitat category (EUNIS code) appearing first in the list of possible habitats was used in this study. Attempting to create senstivity maps for all possible combinations, would cause significant addiitonal computational contrsaints. The first occuring habitat was chosen, as it was the most likely habitat or the msopt abundant within the polygon (Timothy Fegan, Lead Advisor, Marine GI, Evidence Services Natural England, personal comment). For example, if a polygon had EUNIS code A1.1/A1.2, only A1.1 would have been used

```{r habmap200, fig.cap= "Figure 2. The benthic habitat map for the United Kingdom from 0 to 200 NM offshore, showing a small sample of the EUNIS habitat codes." , echo = FALSE, , out.extra='style="background-color: #FFFFFF; padding:20px; display: inline-block;"'}
...
Fine-scale benthic habitats (EUNIS level 4 - 6) which are known to occur within sub-biogeoregions (Figure 4) within (0 - 12 NM) were identified by Dr Keith Hiscock in project for NE (Hiscock, K. 2016). The fine-scale habitats were then associated with the broad-scale habitats within each sub-biogeoregion, where the fine-scale habtiat were 1) known to occur in the given sub-bioregion, and 2) a the assessed habitat is a "child"" fine-scale habitat of a "parent" broad-scale habitat. I.e. the fine-scale habtiat had to be related to the broad-scale habitat, such as a habitat with EUNIS code A5.12 is a parent of A5.127. Each broad-scale (parent) habitat could therefore have multiple fine-scale (child) habitats associated to it within the given subbioregion. The child habitat of the highest EUNIS level and the highest sensitivity was assigned to parent habitats to inform sensitivity assessments.  Sensitivity assessments of higher EUNIS levels were therefore preferred over sensitivity assessments at lower levels, where available. this habitat category replacement was jsutified under the assumption that fine-scale habitats would provide a more realistic assessment of the true sensitivity to a pressure, than the lower EUNIUS level (parent). This was based on the idea that an understandding of the sensitivity of benthic communities and the substrate would be more informative than sensitivity of the substrate alone. This infomration was not currently available for the offshore, but JNCC is working on such a project, and when the data becomes available, it will be incoporated in to the offshore model, as for the inshore.
<br>
```{r subbiogeoregions, fig.cap= "Figure 4: The inshore sub-biogeoregions within which fine-scale habitats (EUNIS level 4 - 6) are known, and were assocaited with broad-scale habitats from which fine scale habitat on the habitat map.",  echo = FALSE, , out.extra='style="background-color: #FFFFFF; padding:20px; display: inline-block;"'}
...
knitr::include_graphics(GIS_process_path, dpi = 1500)
...
### R-code steps
All programming to generate habitat sensitivity layers was carried out in R. the files are stored on Natural England's GitHub [webpage](https://github.com/naturalengland/marine_biotope_sensitivity). Here only the high-level overview of how the code works is described. Details about what each section of code does is broadly described in the Annex, and specific details are described within each R-file. The inshore and offshore are two separate sets of code, and run indecently, after they were split using the GIS described above. The R scripts of r the inshore and offshore follow largely the same steps, described below.

In essence the R-code read the sensitivity assessments (and associated confidence scores) and GIS data into the R statistical environment (an instance of the R programme), where it associated the sensitivity assessments to the respective benthic habitat maps (inshore and offshore). In the offshore the R-code evaluates the fine-scale habitats which may occur within broad-scale habitats, and associates the individual mapped habitat polygons to the fine-scale habitat assessments with the highest sensitivity score. For the offshore, it uses the habitat code provided as is, and does not attempt to find fine-scale habitats within broad-scale habitats, for the reasons provided earlier. The data are then saved and exported to geopackage for the inshore and offshore components, respectively. 

## Case Study
### Fishing activity data investigation
The spatial match of scallop dredging activity was compared to the distribution of sensitive habitats. The habitat sensitivity map to abrasion (pressure code D6) from dredging (activity code Z5) in the inshore area was used. Offshore sensitivity may be added in due course once JNCC data is incoporated in the analysis. The aggregated dredging activity between 2013-2014 stored in file "Fishermap_JNCC_NE/MMO_UK_VMS_Fishing_Activity.gdb" was used as a realistic and recent example of the geographic distribution of dredging pressure in the English EEZ. The above dredging activity data set contains scallop dredging, tractor drawn dredging, and rake dredging data, but the latter two only apply to the shoreline-area. Areas known for other kinds of dredging activity, such as hydrolic dredging at The Wash were excluded after the identification of areas with relatively high fishing pressure in the Results. Areas with relatively high level sof dredging activity were idnetifed by setting a minimum threshold of including cells from the activity map where dredging activity exceeded 20 % of the highest activity value. This was done to restrict the area of investigation to distinct (and disjointed) areas where fishign pressure was greatest. If no cut-off value was applied, the areas become very large and connected along he inshore 12NM boundary.
...
A substantial inshore area was associated with relatively high levels of dredging activity.  The offshore areas between Worthing and Eastbourne was the largest contiguous area with relatively high levels (>10.8) of dredging activity. Dredging around this area was assumed to be largely focussed on scallop dreding, as a large number of scallop dredgers operate from Shoreham Harbour, near Worthing. The Shoreham scallop dredgers effort was predominantly focused in the offshore area (Figure 6). Other areas with relatively high levels of dredging activity thought to be scallop dredging include Portsmouth, Brixham which had highest activity levels inshore. A large area between Falmouth and Plymouth had relatively high levels of dredging activity associated within the inshore area. There was an area with relatively high levels of dredging activity straddled across the inshore and the offshore between Newlyn and the Isles of Scilly. The offshore area between Isle of Man and Blackpool in the Irish Sea was another area with relatively high levels of scallop dredging. These core areas of interest were validated using MMO landings data [website](https://www.gov.uk/government/organisations/marine-management-organisation). See map below for a spatial distribution of catch landings from the MMO Seafisheries report of landings data for 2017 [@dixonUKSeaFisheries2017].

```{r seafish, fig.cap="Figure taken from Landgins distribution of fish in the UK.", echo = FALSE}
seafish_img_path <- "F:/projects/marine_biotope_sensitivity/report/_bookdown_files/_main_files/figure-html/Port_based_landings_no_text_2017.png"
...
2. A fishing activity layer showing dredging and the current Marine Conservation Zone (MCZ) layer shows the placement of MCZs in relation to the fishing activity. The map shows that there was very limited spatial overlap between the dredged areas with high levels of activity (>10.8) and MCZs (Figure 6). 
3. *Marine Conservation Zone (MCZ) layer (file): "Marine_Conservation_Zones_NE_CCW/Marine_Conservation_Zones_England.shp"*
<br>

<br>

```{r dredge, fig.cap= "Figure 5. Dredging activity in English waters from MMO VMS data.", echo = FALSE}
...
The Wash was characterized by having the highest levels of dredging activity in England, however this was thought to be largely based on hydrolic dredging and was therefore not considered further. The Solent area which also had high levels of dredging activity was removed from the study as it was thought not to be focussed around scallop dredging. Cardigan Bay (Bae Ceredigion) had high levels of dredging, but was not further considered as it is outside of English waters.

<br>

```{r dredge_aois, fig.cap="Figure 6. Areas with relatively high levels of dredging actvity between 2009 and 2014 in English fishing waters.", echo = FALSE}

aois_img_path <- "F:/projects/marine_biotope_sensitivity/report/_bookdown_files/_main_files/figure-html/AOI_Fishing_pressure_dredge.png"
...
## Sensitivity mapping
In total about 154 benthic habitat sensitivity mapable data columns were produced for 39 pressures from the ten fishing activities, for the inshore and for the offshore respectively. I.e. including both inshore and offshore 308 habitat sensitivity maps. Note that not all activities-pressure-habitat combinations have valid interactions, and therefore the toal is not 390 per inshore/offshore data set. These mapabale data columns were accompanied by the same number of confidence assesments and assessed fine-scale habtiat type (child of broader habitat type). In total 462 new columns were added to the habitat map, each representing either a senstivity to a activity pressure combination, the associated fine-scale habtiat, and the confidence of the sensitivity assessment. The mapable data columns were stored in two files, one for the offshore, and another for the inshore as geopackages. The geopackages were vector based, and mirror the spatial resolution and habitat detail of the benthic habitat map, as the mapabale columns become attached to the existing habitat map.

The R script which generates the mapabale data set has the capability to instantly produce similar data sets for 18 operations (for the inshore and offshore). The R scripts were not run for all operations, as the outputs would require a significant amount of data storage space (approximately 160 GB). The fact that these maps can and should be updated as new data becomes incorporated into the database and habitat map was sufficient reason to produce the maps only as and when needed the outputs for other activities and operations are requried. 
...
<br>
```{r hab_sens_dredge_abras, fig.cap= "Figure 7: The benthic habitat sensitivity to abrsation from dredging activities in the UK's Exclusive Economic Zone (EEZ)", echo = FALSE}
...
The proportional area per habitat sensitivity, for the inshore and offshore.
<br>
...
prop.area <- readRDS("F:/projects/marine_biotope_sensitivity/report/prop_area_tbl.RDS")
...
``` 
### Large areas within subbioregions and single polygons with high sensitivity
Sub-biogeoregion 2c in the Southern North Sea and 5 in the Irish Sea had the greatest proportional area of highly senstivite habitat each covering about 1.6 % of the total inshore area. The subbiogeoregions 1a and 1b in the northern North Sea had medium sensitivity, and each covered over 30 % of the total inshore area.

In sub-biogeoregion 2c had the largest single polygon of high habitat sensitivity covering an area almost 1.6 % of the inshore habitat. This polygon had a habitat classification of sublittoral mixed sediments (A5.4), which was assessed as fine-scale habitat category *Cerianthus lloydii* and other burrowing anemones in circalittoral muddy mixed sediment (A5.441) against dredging pressure  (Z10_5)from abrasion pressure (D6) and had a high level of senstivity with low degree of confidence in the assessment (Quality of Evidence). the parent habitat type (A5.4) had a medium to  high degree of confidence assocaited with a MESH score  of 66 out of 100. Future analysis will look at contiguous areas (connected polygons) using habitat patch statistics.

The largest non-sensitive area was a polygon off Margate Longsands with habitat type A5.137, Dense *Lanice conchilega* and other polychaetes in tide-swept infralittoral sand and mixed gravelly sand. The assessment had a high level of confidence based on the Quality of the Evidence and a high degree of confidence in the mapped habitat type (MESH score 81). See the sensitivity assessment on the [MarLIN webpage]( https://www.marlin.ac.uk/habitats/detail/116) for more detailed information on the reasons for the non-sensitive habitat type outcome.
Note this this provides an example of onbe kind of activity and pressure.


## Area per habitat sensitivity in areas with "relatively high" levels of fishing pressure. 


*Decide to include or not* 2. Area per habitat sensitivity, in MCZs. (can use fields in fields above and the  to work out the area per marine protected area)


The habitat sensitivity assessments include the assessed habitat, i.e. the fine-scale habitat assocaited with the broad-scale habitats within a subbioregion. 
```{r assessed_habs_inshore, fig.cap= "Figure 8. The assessed fine scale benthic habitats, modelled within the broad-scale habitats in the English territorial (0- 12NM) zone", echo = FALSE}
hab_assessed_img_path <- "F:/projects/marine_biotope_sensitivity/report/_bookdown_files/_main_files/figure-html/inshore_fine_assessed_habs.png"

knitr::include_graphics(hab_assessed_img_path, dpi = 150)
```
<br>

The Quality of Evidence was provided as a spatial measure of confidence of the sensitivity assessments for the inshore and offshore.The confidence map showed that generally the quality of evidence was higher for habitats assessed in the inshore than offshore  areas.
*More analytical detail to be included in future*
<br>
```{r conf_map, fig.cap="Figure 9. Map showing the spatial distribution of confidence based on quality of Evidence for habitat sensitivity assessments in UK waters.", echo = FALSE}
conf_map_img_path <- "F:/projects/marine_biotope_sensitivity/report/_bookdown_files/_main_files/figure-html/confidence_map.png"
include_graphics(conf_map_img_path, dpi = 150)
```

The Quality of Evidence outputs are accompanied by mapable MESH scores, which together provide the user with spatial index of confidence in teh assessments.
...
The R-code steps are outlined below.	
## R-code steps
