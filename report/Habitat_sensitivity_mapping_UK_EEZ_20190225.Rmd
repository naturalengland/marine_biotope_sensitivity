...
Fine-scale benthic habitats (EUNIS level 4 - 6) which are known to occur within sub-biogeoregions within (0 - 12 NM) were identified by Dr Keith Hiscock in project (Hiscock, K. 2016). These fine scale habitats were then assoiated with their parent broad-scale habitats for each sub-biogeoregion. **insert and map and diagram/slide**.

```{r subbiogeoregions, fig.cap= "Figure 3: The subbiogeoregions within which fine-scale habitats (EUNIS level 4 - 6) are known, and were assocaited with broad-scale habitats from which fine scale habitat on the habitat map.",  echo = FALSE}
...
The EUNIS benthic habitat categories which appear on the habitat map were associated with habitat categories contained within the sensitivity assessment database. By joining these datasets sensitivity maps for benthic habitat to the various presssures and activities were generated. The GI and R programming following steps were taken to produce the habitat senstivity maps from these data sources.

### 4. Data processing
GIS steps were required to split the original habitat map into an inshore (0 - 12 NM) and offhsore compoennt (12 - 200 NM). All other technical data manipulation was carried out using the R statistical environment in effort to make the work repeatable and provide instant results following updates to the sensitivity assessment database (PD_AoO.accdb) or updates to the benthic habitat map.
