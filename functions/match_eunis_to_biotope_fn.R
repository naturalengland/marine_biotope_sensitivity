...
match_eunis_to_biotopes_fn <- function(x = x, y = bgr.dfs.lst, high.lvl = mx.lvl) {
        ...
        for (h in seq_along(y)) {
                ...
                #split y[[h]] by eunis "level" to allo associating biotopes with heirarchical levels of eunis codes (to allow using highest level eunis codes in the end)
                ...
                for (k in seq_along(y.dfs.lst)) {
                        z <- y.dfs.lst[[k]]$hab.1 # isolate the habitat variable into a vector, this is what will be matched with x, one at a time
                        ...
                        # generate an empty temporary data frame to store the matched x and z results-----------------------------------
                        result.tbl <- data.frame(matrix(ncol = length(x),nrow = length(z)))
                        ...
                        for (i in seq_along(z)) {# repeats the following commands for each element in y 
                                a <- matrix(ncol = length(x), nrow = length(z)) # generate a matrix with the same number of columns as x, and same number of rows as y in which TRUE or FALSE is stored
                                # repeats for each element in x
                                for (j in seq_along(x))  {
                                        #test if biotope matches the broader EUNIS level category
                                        a <- z[i] %in% substr(x[j],1,nchar.substr) # this return TRUE FALSE
                                ...
                                names(result.tbl) <- as.character(x) #assign column names based on biotope names
