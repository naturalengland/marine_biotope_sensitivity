...
sbgr.matched.btpt.w.rpl <- results.files %>% 
        plyr::llply(function(x){ 
                ...
                # splits it into a list of dataframes according to the assessed (h.lvl) biotope classification level (4/5/6)
                ...
                #repeat the above, but now use EUNIS level 4 to replace any remaining NA values
                ...
                return(l.consolidated) # this is the value that I want to the function to return: send this dataframe (the consolidated dataframe from the three compared dataframes) into the overarching function (this is then repeated for each sbgr)
                
                #to write into separate files remove "#" below, and check that there is a folder called output in the working directory, (getwd()).
                write.csv(l.consolidated, paste0("./sbgr_",unique(l.consolidated$sbgr),"_consolidated_hlvls.csv")) # write the result to file to inspect it, and ensure that R object at the end is correct
                
        }, .parallel = TRUE, .paropts = list(.options.snow=opts), .progress = "text")


cat("Output stored as: sbgr.matched.btpt.w.rpl.   Ignore error messages about RODBC handles that are closed, and 1: <anonymous>: ... may be used in an incorrect context: ‘.fun(piece, ...)’")